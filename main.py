...
def load_data():
    ...
    try:
        ...
        print("❌ خطأ في قراءة ملف البيانات.")
...
async def setup_server_slash(interaction: discord.Interaction):
    ...
    async def confirm_callback(interaction_confirm: discord.Interaction):
        ...
        await interaction_confirm.followup.send("🔄 بدء عملية الإعداد...", ephemeral=True)
        
        guild = interaction_confirm.guild
        
        try:
            # حذف كل القنوات والفئات
            for channel in guild.channels:
                await channel.delete(reason="إعادة إعداد السيرفر")
            
            # حذف كل الرتب (ما عدا @everyone)
            ...
            for role_info in ROLES:
                ...
                created_roles[role_info["name"]] = new_role
            ...
            for category_name, channels in CATEGORIES_AND_CHANNELS.items():
                ...
                if "الدعم الفني" in category_name:
                    ...
                    await ticket_create.send(embed=discord.Embed(title="🎫 نظام التذاكر", description="اضغط على الزر لإنشاء تذكرة", color=INFO_COLOR), view=TicketView())
                    await guild.create_text_channel("📋・التذاكر-المفتوحة", category=category, topic="قائمة التذاكر المفتوحة")
            ...
            success_embed = discord.Embed(title="✅ تم إعداد السيرفر بنجاح", description="تم إنشاء جميع الرتب والقنوات.", color=SUCCESS_COLOR)
            await interaction_confirm.followup.send(embed=success_embed, ephemeral=True)
